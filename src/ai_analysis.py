...
ERROR_WEIGHTS = {
    'meaning': 0.4,      # Потеря смысла - самый важный фактор
    'key_words': 0.3,    # Неправильный перевод ключевых слов
    'grammar': 0.15,     # Грамматические ошибки
    'punctuation': 0.05, # Пунктуация
    'style': 0.1         # Стилистические различия
}
...
"""
        ...
        prompt += """
ВАЖНО: Используй гибридную систему оценки с детальным анализом ошибок.
...
- 0.7-0.8: Хорошо, смысл понят, незначительные ошибки
- 0.9-1.0: Отлично, смысл понят полностью

АНАЛИЗИРУЙ КАЖДЫЙ ТИП ОШИБКИ:

1. СМЫСЛОВЫЕ ОШИБКИ (вес 40%):
...
2. ЛЕКСИЧЕСКИЕ ОШИБКИ (вес 30%):
   ...
   - Использование неточных синонимов
   - Пропуск важных слов

3. ГРАММАТИЧЕСКИЕ ОШИБКИ (вес 15%):
   - Неправильное время глагола
   - Ошибки в согласовании
   - Неправильный порядок слов

4. ПУНКТУАЦИОННЫЕ ОШИБКИ (вес 5%):
   - Отсутствие запятых
   - Неправильные знаки препинания

5. СТИЛИСТИЧЕСКИЕ ОТЛИЧИЯ (вес 10%):
   - Различия в стиле
...
"""
        ...
        prompt += """
ВАЖНО: Используй гибридную систему оценки с детальным анализом ошибок для перевода на английский.
...
Твоя главная задача - оценить, насколько правильно студент понял СМЫСЛ фразы, используя гибридную систему оценки.

ПРИОРИТЕТЫ ОЦЕНКИ (по важности):
1. Понимание основного смысла фразы (40% веса)
2. Точность перевода ключевых слов (30% веса)
3. Грамматическая корректность (15% веса)
4. Стилистические различия (10% веса)
5. Пунктуация (5% веса)
...
- 0.7-0.8: Хорошее понимание с мелкими ошибками
- 0.9-1.0: Отличное понимание

АНАЛИЗИРУЙ КАЖДЫЙ ТИП ОШИБКИ ОТДЕЛЬНО:
- Смысловые ошибки: полная потеря или искажение смысла
- Лексические ошибки: неправильный перевод ключевых слов
- Грамматические ошибки: время, согласование, порядок слов
- Пунктуационные ошибки: запятые, знаки препинания
- Стилистические отличия: синонимы, перефразировки

БУДЬ СПРАВЕДЛИВЫМ:
- Если смысл понят правильно, ставь высокий балл (0.8-1.0)
- За похожие переводы снижай балл незначительно (0.1-0.2)
- За ошибку в 1 слове снижай балл умеренно (0.2-0.3)
- Детально объясняй каждый тип ошибки в error_analysis
...
- Смысловые ошибки: полная потеря или искажение смысла
- Лексические ошибки: неправильный перевод ключевых слов
- Грамматические ошибки: время, согласование, порядок слов, артикли
- Пунктуационные ошибки: запятые, знаки препинания
- Стилистические отличия: синонимы, перефразировки
...
Давай конкретные советы по исправлению ошибок, а не общие рекомендации."""
    ...
    def _get_fallback_analysis(self, user_answer: str, correct_answer: str) -> Dict[str, any]:
        ...
        if user_lower == correct_lower:
            ...
            if user_answer.replace(' ', '') != correct_answer.replace(' ', ''):
                if not any(p in user_answer for p in '.,!?;:'):
                    punctuation_errors.append("Отсутствуют знаки препинания")
    ...
    def _get_fallback_reverse_analysis(self, user_answer: str, correct_answer: str) -> Dict[str, any]:
        ...
        if user_lower == correct_lower:
            ...
            # Проверяем артикли (специфично для английского)
            if 'the' in correct_lower and 'the' not in user_lower:
                grammar_errors.append("Отсутствует артикль 'the'")
            elif 'a' in correct_lower and 'a' not in user_lower:
                grammar_errors.append("Отсутствует артикль 'a'")
